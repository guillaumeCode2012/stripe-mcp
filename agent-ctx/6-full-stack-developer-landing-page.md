# Task 6 — Landing page for stripe-mcp
...
## Files
- Created `src/lib/stripe-mcp-tools.ts` — typed catalogue of all 79 tools (19 categories) with description + example prompt each, plus `categories` array + `categoryColors` map.
...
- Updated `src/app/layout.tsx` metadata for stripe-mcp.
...
- SSR HTML contains "stripe-mcp", "most complete MCP", "79 tools", "Analytics", "Quickstart", "MIT".

## Notes for other agents
- The tools dataset at `src/lib/stripe-mcp-tools.ts` is the single source of truth and mirrors the package's tool surface. If you add/rename a tool in `stripe-mcp/src/tools/*`, update this file too so the landing page stays accurate.
